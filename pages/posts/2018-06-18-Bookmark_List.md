# Bookmark List

## Type Class
+ [Down with Show! Rules of thumb for when to use a type class Series](https://harry.garrood.me/blog/down-with-show-part-1/)
+ [What are type families?](https://typesandkinds.wordpress.com/2015/09/09/what-are-type-families/)
  
## Type-Level Programming
+ [Fun with Typed Type-Level Programming in PureScript](https://blog.wuct.me/fun-with-typed-type-level-programming-in-purescript-5f8af42cfec5)
+ [Generalized algebraic datatypes](https://en.wikibooks.org/wiki/Haskell/GADT)
+ [Type Families and Pokemon](https://www.schoolofhaskell.com/school/to-infinity-and-beyond/pick-of-the-week/type-families-and-pokemon)
+ [Dependently typed servers in Servant](https://www.well-typed.com/blog/2015/12/dependently-typed-servers/)
+ [Why Dependent Haskell is the Future of Software Development](https://serokell.io/blog/2018/12/17/why-dependent-haskell)
   
## Template Haskell

+ [Template Haskell Wiki](https://wiki.haskell.org/Template_Haskell)
+ [Template Haskell tutorial](https://markkarpov.com/tutorial/th.html) 
+ [A practical Template Haskell Tutorial](https://wiki.haskell.org/A_practical_Template_Haskell_Tutorial)

## Free-Monad, GADTs and Tagless Final Style
+ [Introduction to Tagless Final](https://serokell.io/blog/2018/12/07/tagless-final)
+ [Free Monad vs Tagless Final](https://medium.com/@agaro1121/free-monad-vs-tagless-final-623f92313eac)
+ [Typed final (tagless-final) style](http://okmij.org/ftp/tagless-final)
+ [GADTs - Haskell for All](http://www.haskellforall.com/2012/06/gadts.html)
+ [Anthony Cowley - Framing the Discussion with EDSLs (Video)](https://www.youtube.com/watch?v=_KioQRICpmo)
+ [TIAP Bob Atkey - An Algebraic Approach to Typechecking and Elaboration (Video)](https://www.youtube.com/watch?v=R5NMX8FBlWU)

## GHC Extensions
+ [Introduction to Pattern Synonyms](https://www.schoolofhaskell.com/user/icelandj/Pattern%20synonyms)
